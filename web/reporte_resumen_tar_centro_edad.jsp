...
try{
    if (!conexion.isClosed()){
      ...
      rs_tar = st.executeQuery("SELECT UPPER(Zona.Nombre) AS ZonaNombre, UPPER(Centro.ServicioSalud) AS ServicioSalud, UPPER(Centro.Nombre) AS CentroNombre, Centro.ID as CentroID, COUNT(*) AS Total, COUNT(if(Paciente.SexoID=1, 1, NULL)) AS Femenino, COUNT(if(Paciente.SexoID=2, 1, NULL)) AS Masculino "+
                                     ...
                                     "GROUP BY Zona.Nombre, Centro.ServicioSalud, Centro.Nombre "+
                                     "ORDER BY Zona.Orden, Centro.Orden;");
    }
}catch(Exception e){
}
%>
...
<html>
    <head>
        ...
        <script>
            $( document ).ready(function() {
                $(".detalle_resumen").hide();
                $(".click_detalle").click(function(){
                    var centroid = $(this).attr("centroid");
                    $(".detalle_resumen[centroid='"+centroid+"']").toggle();
                });
            });
        </script>
        <title>Reporte Pacientes Agrupados por Centro y Rango Edad al Ingreso a Centro</title>
    ...
    <body>
        ...
        <div id="content" class="left_content">
            <h1>Reporte Pacientes Agrupados por Centro y Rango Edad al Ingreso a Centro</h1>
            ...
            <div class="ui-datatable-tablewrapper">
                ...
                <thead id="j_idt85:tbl_head">
                    <tr role="row">
                        ...
                        <th class="ui-state-default" role="columnheader"><span class="ui-column-title">Centro</span></th>
            ...
            <%
                if(rs_tar!=null){
                while (rs_tar.next()){
                    String CentroID = String.valueOf(rs_tar.getObject("CentroID"));
                    out.println("<tr role=\"row\" style=\"background-color:#EEEEEE; cursor:pointer;\" class=\"click_detalle\" centroid=\""+CentroID+"\">");
                    ...
                    out.println("<td>"+String.valueOf(rs_tar.getObject("CentroNombre"))+"</td>");
                    ...
                    // Detalle Rango Edad
                    Statement st = conexion.createStatement();
                    ResultSet rs_edad = st.executeQuery("SELECT '15-19' AS Edad, COUNT(*) AS Total, COUNT(if(Paciente.SexoID=1, 1, NULL)) AS Femenino, COUNT(if(Paciente.SexoID=2, 1, NULL)) AS Masculino "+
                                          "FROM cohorte.Paciente "+
                                          "WHERE Paciente.CentroID = "+CentroID+" AND EdadIngreso IS NOT NULL  "+
                                          "        AND EdadIngreso >= 15  "+
                                          "        AND EdadIngreso <=19 "+
                                          "        AND Paciente.ID IN (SELECT PacienteID FROM Terapia) "+
                                          "UNION "+
                                          "SELECT '20-24', COUNT(*), COUNT(if(Paciente.SexoID=1, 1, NULL)), COUNT(if(Paciente.SexoID=2, 1, NULL))  "+
                                          "FROM cohorte.Paciente "+
                                          "WHERE Paciente.CentroID = "+CentroID+" AND EdadIngreso IS NOT NULL  "+
                                          "        AND EdadIngreso >= 20  "+
                                          "        AND EdadIngreso <= 24 "+
                                          "        AND Paciente.ID IN (SELECT PacienteID FROM Terapia) "+
                                          "UNION "+
                                          "SELECT '25-29', COUNT(*), COUNT(if(Paciente.SexoID=1, 1, NULL)), COUNT(if(Paciente.SexoID=2, 1, NULL))  "+
                                          "FROM cohorte.Paciente "+
                                          "WHERE Paciente.CentroID = "+CentroID+" AND EdadIngreso IS NOT NULL  "+
                                          "        AND EdadIngreso >= 25  "+
                                          "        AND EdadIngreso <= 29 "+
                                          "        AND Paciente.ID IN (SELECT PacienteID FROM Terapia) "+
                                          "UNION "+
                                          "SELECT '30-39', COUNT(*), COUNT(if(Paciente.SexoID=1, 1, NULL)), COUNT(if(Paciente.SexoID=2, 1, NULL))  "+
                                          "FROM cohorte.Paciente "+
                                          "WHERE Paciente.CentroID = "+CentroID+" AND EdadIngreso IS NOT NULL  "+
                                          "        AND EdadIngreso >= 30 "+
                                          "        AND EdadIngreso <= 39 "+
                                          "        AND Paciente.ID IN (SELECT PacienteID FROM Terapia) "+
                                          "    UNION "+
                                          "SELECT '40-49', COUNT(*), COUNT(if(Paciente.SexoID=1, 1, NULL)), COUNT(if(Paciente.SexoID=2, 1, NULL))  "+
                                          "FROM cohorte.Paciente "+
                                          "WHERE Paciente.CentroID = "+CentroID+" AND EdadIngreso IS NOT NULL  "+
                                          "        AND EdadIngreso >= 40  "+
                                          "        AND EdadIngreso <= 49 "+
                                          "        AND Paciente.ID IN (SELECT PacienteID FROM Terapia) "+
                                          "UNION "+
                                          "SELECT '50-59', COUNT(*), COUNT(if(Paciente.SexoID=1, 1, NULL)), COUNT(if(Paciente.SexoID=2, 1, NULL))  "+
                                          "FROM cohorte.Paciente "+
                                          "WHERE Paciente.CentroID = "+CentroID+" AND EdadIngreso IS NOT NULL  "+
                                          "        AND EdadIngreso >= 50 "+
                                          "        AND EdadIngreso <= 59 "+
                                          "        AND Paciente.ID IN (SELECT PacienteID FROM Terapia) "+
                                          "    UNION "+
                                          "SELECT '60-69', COUNT(*), COUNT(if(Paciente.SexoID=1, 1, NULL)), COUNT(if(Paciente.SexoID=2, 1, NULL))  "+
                                          "FROM cohorte.Paciente "+
                                          "WHERE Paciente.CentroID = "+CentroID+" AND EdadIngreso IS NOT NULL  "+
                                          "        AND EdadIngreso >= 60  "+
                                          "        AND EdadIngreso <= 69 "+
                                          "        AND Paciente.ID IN (SELECT PacienteID FROM Terapia) "+
                                          "UNION "+
                                          "SELECT '70+', COUNT(*), COUNT(if(Paciente.SexoID=1, 1, NULL)), COUNT(if(Paciente.SexoID=2, 1, NULL))  "+
                                          "FROM cohorte.Paciente "+
                                          "WHERE Paciente.CentroID = "+CentroID+" AND EdadIngreso IS NOT NULL  "+
                                          "        AND EdadIngreso >= 70 "+
                                          "        AND Paciente.ID IN (SELECT PacienteID FROM Terapia)");
                    if(rs_edad!=null){
                        while (rs_edad.next()){
                            out.println("<tr role=\"row\" class=\"detalle_resumen\" centroid=\""+CentroID+"\">");
                            out.println("<td colspan=\"3\" align=\"right\">"+String.valueOf(rs_edad.getObject("Edad"))+"</td>");
                            out.print("<td align=\"right\">");
                            {
                                DecimalFormatSymbols unusualSymbols = new DecimalFormatSymbols(Locale.US);
                                unusualSymbols.setDecimalSeparator(',');
                                ...
                                DecimalFormat weirdFormatter = new DecimalFormat(strange, unusualSymbols);
                                weirdFormatter.setGroupingSize(3);
                                out.print(weirdFormatter.format(Integer.parseInt(String.valueOf(rs_edad.getObject("Total")))));
                            }
                            out.println("</td>");
                            ...
                            out.print("<td align=\"right\">");
                            {
                                ...
                                DecimalFormat weirdFormatter = new DecimalFormat(strange, unusualSymbols);
                                weirdFormatter.setGroupingSize(3);
                                out.print(weirdFormatter.format(Integer.parseInt(String.valueOf(rs_edad.getObject("Femenino")))));
                            }
                            out.println("</td>");
                            ...
                            out.print("<td align=\"right\">");
                            {
                                ...
                                DecimalFormat weirdFormatter = new DecimalFormat(strange, unusualSymbols);
                                weirdFormatter.setGroupingSize(3);
                                out.print(weirdFormatter.format(Integer.parseInt(String.valueOf(rs_edad.getObject("Masculino")))));
                            }
                            out.println("</td>");
                            out.println("</tr>");
                        }
