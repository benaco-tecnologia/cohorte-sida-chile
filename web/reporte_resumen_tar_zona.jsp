...
try{
    if (!conexion.isClosed()){
      ...
      rs_tar = st.executeQuery("SELECT UPPER(Zona.Nombre) AS ZonaNombre, Zona.ID AS ZonaID, COUNT(*) AS Total, COUNT(if(Paciente.SexoID=1, 1, NULL)) AS Femenino, COUNT(if(Paciente.SexoID=2, 1, NULL)) AS Masculino "+
...
<html>
        ...
        <script>
            $( document ).ready(function() {
                ...
                $(".click_detalle").click(function(){
                    var zonaid = $(this).attr("zonaid");
                    $(".detalle_resumen[zonaid='"+zonaid+"']").toggle();
                });
            });
        </script>
        <title>Reporte Pacientes Agrupados por Zona con Terapia Iniciada</title>
    ...
    <body>
        ...
        <div id="content" class="left_content">
            <h1>Reporte Pacientes Agrupados por Zona con Terapia Iniciada</h1>
            ...
            <%
                if(rs_tar!=null){
                while (rs_tar.next()){
                    String ZonaID = String.valueOf(rs_tar.getObject("ZonaID"));
                    out.println("<tr role=\"row\" style=\"background-color:#EEEEEE; cursor:pointer;\" class=\"click_detalle\" zonaid=\""+ZonaID+"\">");
                    ...
                    // Detalle Zona por SS
                    Statement st = conexion.createStatement();
                    ResultSet rs_ss = st.executeQuery("SELECT UPPER(Centro.ServicioSalud) AS ServicioSalud, COUNT(*) AS Total, COUNT(if(Paciente.SexoID=1, 1, NULL)) AS Femenino, COUNT(if(Paciente.SexoID=2, 1, NULL)) AS Masculino "+
                                     "FROM Paciente, Centro "+
                                     "WHERE Centro.ServicioSalud IS NOT NULL "+
                                            "AND Paciente.ID IN (SELECT PacienteID FROM Terapia WHERE Terapia.NumeroTar = 1) "+
                                            "AND Paciente.CentroID = Centro.ID "+
                                      " GROUP BY Centro.ZonaID, Centro.ServicioSalud "+
                                     " HAVING Centro.ZonaID = "+ZonaID+" "+
                                     "ORDER BY Centro.Orden;");
                    if(rs_ss!=null){
                        while (rs_ss.next()){
                            out.println("<tr role=\"row\" class=\"detalle_resumen\" zonaid=\""+ZonaID+"\">");
                            out.println("<td>"+String.valueOf(rs_ss.getObject("ServicioSalud"))+"</td>");
                            out.print("<td align=\"right\">");
                            {
                                DecimalFormatSymbols unusualSymbols = new DecimalFormatSymbols(Locale.US);
                                unusualSymbols.setDecimalSeparator(',');
                                ...
                                DecimalFormat weirdFormatter = new DecimalFormat(strange, unusualSymbols);
                                weirdFormatter.setGroupingSize(3);
                                out.print(weirdFormatter.format(Integer.parseInt(String.valueOf(rs_ss.getObject("Total")))));
                            }
                            out.println("</td>");
                            ...
                            out.print("<td align=\"right\">");
                            {
                                ...
                                DecimalFormat weirdFormatter = new DecimalFormat(strange, unusualSymbols);
                                weirdFormatter.setGroupingSize(3);
                                out.print(weirdFormatter.format(Integer.parseInt(String.valueOf(rs_ss.getObject("Femenino")))));
                            }
                            out.println("</td>");
                            ...
                            out.print("<td align=\"right\">");
                            {
                                ...
                                DecimalFormat weirdFormatter = new DecimalFormat(strange, unusualSymbols);
                                weirdFormatter.setGroupingSize(3);
                                out.print(weirdFormatter.format(Integer.parseInt(String.valueOf(rs_ss.getObject("Masculino")))));
                            }
                            out.println("</td>");
                            out.println("</tr>");
                        }
            ...
            %>
                    ...
                    <tr role="row">
                        <td class="ui-state-default" role="" align="right"><span class=""></span></td>
                        <td class="ui-state-default" role="" align="right"><span class=""><strong>TPacientes</strong></span></td>
                        <td class="ui-state-default" role="" align="right"><span class=""><strong>TMujeres</strong></span></td>
                        <td class="ui-state-default" role="" align="right"><span class=""><strong>THombres</strong></span></td>
                    </tr>
               </tbody>
                </table>
            </div>
